...
if input("Forecast? y/n") == 'y':
	inputData = inputData[:SPLIT_TIME]
	inputData = np.array(inputData)
	timeData = timeData[:SPLIT_TIME]
	timeData = np.array(timeData)
	for i in range(0,FORECAST_TIMESTEPS):
		cnn_forecast = model_forecast(model, inputData[SPLIT_TIME + i-WINDOW_SIZE:][..., np.newaxis], WINDOW_SIZE)
	...
	plot_series(timeData[SPLIT_TIME:], inputData[SPLIT_TIME:])
